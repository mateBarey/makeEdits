...
class wsAssistant:
    ...
    async def _create_and_start_connection(self, channel, ticker=None):
        ...
        async with anyio.create_task_group() as tg:
            await tg.spawn(self.make_ws_req,channel,req,i,tickers_,obooks_)
            self.active_connections[uid] = [ws_connection, req,tg]
  ...
  # Modify the pause and resume methods to handle all tickers
    def pause_all_make_ws_req(self):
        ''' Pause all make_ws_req coroutines '''
        for uid in self.active_connections:
           self.task_manager.pause_task(uid)
        

    def resume_required_make_ws_req(self):
        ''' Resume the make_ws_req coroutines for required tickers '''
        for uid in self.active_connections:
            self.task_manager.resume_task(uid)

    async def _disconnect_and_remove(self, uid):
        """Disconnect and remove a connection and its associated subprocess."""
        ws_connection, _, tg = self.active_connections.get(uid, [None, None, None, []])
        if ws_connection :
    ...
    async def monitor_tickers(self):
        '''monitor tickers within ws_obook_posns for addn and removal'''
        current_tickers = {v[1]['id']
                           for k, v in self.active_connections.items() if 'id' in v[1]}
        ...
        # Start connections for new tickers using comprehension
        if new_tickers and not self.add_tickers:
            self.add_tickers = True 
            [asyncio.create_task(self.start_new_connection_for_ticker(
                ticker,self.channels[0])) for ticker in new_tickers]

        if removed_tickers and not self.remove_tickers:
            self.remove_tickers = True 
            await self.flush_producer() #
            self.orderbook_batch = None
            self.tickers_in_batch.clear()

            try:
                # Stop connections for removed tickers using comprehension
                
                await asyncio.gather(*[self.stop_connection_for_ticker(ticker) for ticker in removed_tickers])
    ...
    async def make_req_(self, i, channel, res, tickers_, obooks_):
        if not self.add_tickers:
            tickers_added, obooks_ = await self.add_ticker_channel_specific_params(channel,tickers_added)
        '''routing fn for routing each specific ws response to its appropriate analysis fn'''
        obooks = [self.obook_select(obooks_, ticker) for ticker in tickers_]
        eval_tickers = [self.select_channel_analysis(i, channel, res, ticker, obooks[i1])
                        for i1, ticker in enumerate(tickers_)]
        eval_tickers = await gather(*eval_tickers)
    ...
    async def make_ws_req(self, channel, req, i, tickers_, obooks_):
        ...
        async with self.semaphore:
            ...
            tg = self.active_connections[uid][2]
            try:
                while await self._while_condn_for_channel(channel,ticker):
                    ...
                    try:
                        ...
                        if 'contents' in res:
                            # print(f'res: {res}')
                            sub_task = tg.start_soon(self.make_req_,i, channel, res, tickers_, obooks_)
    ...
    async def get_web_socket_feed(self, channel):
        ...
        if channel in channels_w_id:
            tasks = [asyncio.create_task(self._create_and_start_connection(channel,ticker)) for ticker in tickers_] #mult reqs - orderbook or trades
